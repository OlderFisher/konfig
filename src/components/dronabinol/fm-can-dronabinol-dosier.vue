<template>
	...
	<div class="row form-group fm-can-dosier-inputs-block pt-3 pb-2 ps-4 pe-4 mb-1">
		<div class="col-12 col-md-6" v-for="(field,i) in getDronabinolInputs" :key="i">
	...
	<fm-can-final-message :showStatement="getDronabinolFinalMessage"></fm-can-final-message>
...
export default ({
	...
	computed:{
		...mapGetters(['getDronabinolInputs','getDronabinolFinalMessage']),
	},
	emits:[
		'stepgone'
	],
	...
	methods: {
		onInput($event) {
			...
			if( val.match(/^[0-9]*([.,][0-9]+)?$/)){
				...
				this.$store.commit('dronInputDose',this.outputValues) ;
			}

			if( this.outputValues[0] && this.outputValues[1]){
				this.$store.commit('setDronabinolFinalMessage',true) ;
				this.$emit('stepgone',{
                    accpart : 1 ,
                    accstate : true,
                   }	    
				...
				this.$store.commit('setDronabinolFinalMessage',false) ;
				this.$emit('stepgone',{
                    accpart : 1 ,
                    accstate : false,
                }
