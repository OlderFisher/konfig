...
export default ({
	...
	data() {
		return {
			accElements: [
				{ active: true,  title: 'Menge Und Abgabeform', id: '1', component:'FmCanCannaxanMenge' , disabled : false, step : false},
				{ active: false, title: 'Dosierung', 			id: '2', component:'FmCanCannaxanDosier', disabled : true, step : false },
			],
			steps : 0 ,
		}
	},
	methods: {
		...
		enableCardButtons($event){
			this.$store.commit('setCannaxanCardBlockStatement',$event);
			// this.$emit('enabledbuttons',$event);
		},
		stepGone($event){

			console.log('stepgone');
			// set steps to real state
			if( this.$store.getters.getCannaxanSlider.value > 0 ){
				this.accElements[0].step = true
			}else{
				this.accElements[0].step = false
			}
			if( this.$store.getters.getCannaxanInputs[0].value && this.$store.getters.getCannaxanInputs[1].value){
			...
			if ( $event.accstate == false ){
				...
				if( ($event.accpart + 1) < this.accElements.length ){
					...
					this.accElements[$event.accpart + 1].disabled = false ;
