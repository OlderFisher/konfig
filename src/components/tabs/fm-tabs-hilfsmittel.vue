...
export default ({
	...
	data() {
		return {
			accElements: [
				{ active: true, title: 'Vaporizer Und Zubehör', 	id: '1', component: 'FmCanHilfVaporizer' , disabled : false, step : false},
				{ active: false, title: 'Diagnose', 				id: '2', component: 'FmCanHilfDiagnose'  , disabled : true, step : false },
			]
		}
	},
	methods: {
		...
		enableCardButtons($event){
			this.$store.commit('setHilfsmittelCardBlockStatement',$event);
			// this.$emit('enabledbuttons',$event);
		},
		stepGone($event){

			if( this.$store.getters.getTotalGadgetsAmount > 0 ){
				this.accElements[0].step = true
			}else{
				this.accElements[0].step = false
			}
			if( this.$store.getters.getGadgetDiagnose.length > 5){
