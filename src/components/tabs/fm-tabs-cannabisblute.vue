...
export default ({
	...
	data() {
		return {
			accElements : [
				{ active: true , title: 'Sorte Auswählen' 	  , id: '1' , component: 'FmCanCanbluteSorte' , disabled : false, step : false},
				{ active: false, title: 'Menge Und Abgabeform', id: '2' , component: 'FmCanCanbluteMenge' , disabled : true, step : false},
				{ active: false, title: 'Dosierung', 			id: '3' , component: 'FmCanCanbluteDosier', disabled : true, step : false},
			]
		}
	},
	methods: {
		...
		enableCardButtons($event){
			this.$store.commit('setBluteCardBlockStatement',$event);
		},
		stepGone($event){

			if( this.$store.getters.getBluteSorteActive.length > 0){
				this.accElements[0].step = true
			}else{
				this.accElements[0].step = false
			}
			if( this.$store.getters. getBluteMengeSlider.value > 0){
				this.accElements[1].step = true
			}else{
				this.accElements[1].step = false
			}
			if( this.$store.getters.getBluteDosierInputFields[0].value && this.$store.getters.getBluteDosierInputFields[1].value || this.$store.getters.getBluteDosierTextarea.length > 5){
