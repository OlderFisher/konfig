...
export default ({
	...
	data() {
		return {
			accElements: [
				{ active: true, title: 'Sorte Auswählen',	id: '1', component: 'FmCanOligeSorte' ,disabled : false, step : false},
				{ active: false, title: 'Dosierung', 		id: '2', component: 'FmCanOligeDosier',disabled : true, step : false },
			]
		}
	},
	methods: {
		...
		enableCardButtons($event){
			this.$store.commit('setOligeCardBlockStatement',$event);
		},
		stepGone($event){
			
			if( this.$store.getters.getOligeSorteSlider.value > 0 ){
				this.accElements[0].step = true
			}else{
				this.accElements[0].step = false
			}
			if( this.$store.getters.getOligeDosierInputs[0].value && this.$store.getters.getOligeDosierInputs[1].value || this.$store.getters.getOligeTextArea.length > 5){
