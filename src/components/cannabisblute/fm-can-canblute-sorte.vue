<template>
    <fm-can-warning-desc>
        <div class="fm-fnt-300-12-16 fm-fnt-black">
            ...
            ein <span class="fm-fnt-red">Rezepturarzneimittel</span> verordnet werden. Das bedeutet, dass für jede Blütensorte ein eigenes 
            BtM-Rezept ausgestellt werden sollte.
        </div>
    </fm-can-warning-desc>

    <fm-can-blute-filter :canblutelist="canBluteList" @filterchanged="applyFilter($event.filter)"></fm-can-blute-filter>

    <div class="row fm-can-heading fm-fnt-500-16-22 fm-fnt-black ps-4"  style="margin-top:10px; margin-bottom:16px">
        <div class="col-12">
            Bitte wählen Sie eine Blute aus der Liste
        </div>
    </div>
    
    <fm-can-output-blute-table :canlist="canBluteList" @setactivextrakt="setActiveExtr($event.index)">
    </fm-can-output-blute-table>
...
import FmCanBluteFilter from '@/components/cannabisblute/fm-can-blute-filter.vue'
import FmCanOutputBluteTable from '@/components/cannabisblute/fm-can-output-blute-table.vue'
import config from "/src/config.js";

export default ({
     async created() {  
        this.canBluteOriginal = this.$store.getters.getAllBlutes ;
        this.canBluteList = this.$store.getters.getAllBlutes ;
    },
    mounted(){
        let activeProduct = this.$store.getters.getBluteSorteActive ;
        for( let i= 0 ; i < this.canBluteList.length ; i++ ){
            if(this.canBluteList[i].productname == activeProduct)
    ...
    methods : {
        setActiveExtr(i){
            this.canBluteList.forEach(function(item){
                item.active=false ;
            });
            this.canBluteList[i].active = true ;
            this.greenButtonState = true ;
           
            let product = this.canBluteList[i].productname ;
            this.$store.commit('setActiveBlute',product) ;
        },
        stepGone(){
		this.$emit('stepgone',{
			accpart : 0 ,
        ...
        applyFilter(filterConds){
            ...
            let arrByTHCCBD = [];
            for (let extract of this.canBluteOriginal) {
            ...
            this.canBluteList = JSON.parse(JSON.stringify(arrBySearch));
        },
    },
