    ...
    <div class="row fm-can-blute-filter fm-fnt-500-16-22 fm-fnt-black  ps-4 ">
        <div class="col-12 ps-3">
            <div class="fm-can-blute-filter-heading" @click="setView" @mouseover="onHover" @mouseout="outHover">
                ...
                <div class="fm-can-blute-filter-heading-header fm-fnt-500-16-21 fm-fnt-green ms-2">Filter Cannabisblüte
                </div>
            </div>
        </div>
    </div>
    <div v-if="!viewMobile" class="fm-can-all-filters">
        ...
        <div class="row fm-can-blute-filter fm-fnt-500-16-22 fm-fnt-black ps-4 pb-0">
            <div class="col-12 col-md-6 ps-3 pe-4">
                <fm-can-filter-search @searchfiltervalue="setSearch($event.value)"></fm-can-filter-search>
            </div>
            <div class="col-12 col-md-6 ps-3  pe-4">
        ...
        <div class="row fm-can-blute-filter fm-fnt-500-16-22 fm-fnt-black  ps-4 pb-1">
            ...
            <div class="col-12 col-md-6 ps-0 ps-md-1 pe-4">
...
import FmCanFilterValueSlider from '@/components/cannabisblute/filters/fm-can-filter-value-slider.vue'
import FmCanFilterSelect from '@/components/cannabisblute/filters/fm-can-filter-select.vue'
import FmCanFilterRadioVorrat from '@/components/cannabisblute/filters/fm-can-filter-radio-vorrat.vue'
import FmCanFilterSearch from '@/components/cannabisblute/filters/fm-can-filter-search.vue'
import FmCanFilterCheckgroup from '@/components/cannabisblute/filters/fm-can-filter-checkgroup.vue'
...
export default {
    ...
    props: {
        canblutelist: {type: Object, required: true}
    },
    emits : [
      'filterchanged'
    ],
    data() {
        return {
            ...
            sliderthc: { startvalue: 0, endvalue: 0, note: 'THC (< %)', value : 0},
            slidercbd: { startvalue: 0, endvalue: 0, note: 'CBD (< %)', value : 0},
            companies: [],
            filter: {
                search: '',
    ...
    methods: {
        ...
        setTHCSlider() {
            ...
            for (let i = 0; i < this.canblutelist.length; i++) {
                let thc = this.canblutelist[i].thc * 1;
                thcArr.push(thc);
            }
        ...
        setCBDSlider() {
            let cbdArr = [];
            for (let i = 0; i < this.canblutelist.length; i++) {
                let cbd = this.canblutelist[i].cbd * 1;
                cbd = isNaN(cbd) ? 1 : cbd;
                cbdArr.push(cbd);
            }
        ...
        setCompaniesList(){
            ...
            var index = 0 ;
            for (let element of this.canblutelist) {
                if ( !isCompany.includes(element.brand.label) ) {
                    let company = {id: index, name: element.brand.label}
                    this.companies.push(company);
                    isCompany.push(element.brand.label) ;
                    index++ ;
                }
            }
        },
